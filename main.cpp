...
int main(int argc, char* argv[])
{
		...
		for (i = tempindex = 0;i <= length;i++)
		{
			if (a[i] == ' ' || a[i] == '\n' || a[i] == '\0')
			{
				if (tempindex == 0)
				{
					...
					if (NULL != (subline[linenum]->getLast())) {
						//cout<<subline[linenum]->getLast()->getName()<<endl;
						subline[linenum]->getLast()->addNear(j, linenum);
						sta[j]->addNear(subline[linenum]->getLast()->getNo(), linenum);
					}
	...
	if (argc == 2)
	{
		for (i = 0;i<linenum;i++)
		{
			if (strcmp(subline[i]->getName(), argv[1]) == 0)
			{
				subline[i]->printsta();
				return 0;
			}
...
void shortestPath(int start, int end)
{
	...
	int distance[1000];
	int prepoints[maxnearbynum];
	int prelength;
	int nowpoints[maxnearbynum];
	int nowlength;
	int temppoint;
	int i, j, length;
	for (i = 0;i<1000;i++)
	...
	for (nowlength = distance[end] - 1;nowlength >= 0;nowlength--) {
		length = sta[temppoint]->getNearbynum();
		for (i = 0;i<length;i++)
		{
			if (distance[sta[temppoint]->getNearbysta(i)] == nowlength) {
				if (nowsubline == -1) {
					nowsubline = sta[temppoint]->getNearbypath(i);
					cout << sta[temppoint]->getName() << endl;
					temppoint = sta[temppoint]->getNearbysta(i);
				}
				else if (nowsubline != sta[temppoint]->getNearbypath(i))
				{
					nowsubline = sta[temppoint]->getNearbypath(i);
...
void printPath(int start, int end)
{
	...
	while (true) {
		for (i = 0;i<length;i++)
		{
			...
			{
				if (sta[points[i]]->getNearbypath(j) == linelist[i]) {
					for (k = 0;k<temp;k++)
						if (ways[i][k] == sta[points[i]]->getNearbysta(j))
							break;
					if (k == temp)
						ways[i][temp] = sta[points[i]]->getNearbysta(j), points[i] = ways[i][temp];
				}
			}
		}
		if (flag)
