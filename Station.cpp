...
int Station::addNearBack(int nearstaNo, int sublineNo)
{
	if (nearbybacknum >= maxnearbynum)
		return 1;
	nearbybacksta[nearbybacknum][0] = nearstaNo;
	nearbybacksta[nearbybacknum][1] = sublineNo;
	nearbybacknum++;
	return 0;
}
...
int Station::getNearbyBacknum()
{
	return nearbybacknum;
}
...
int Station::getNearbyBacksta(int i)
{
	if (i >= nearbybacknum)
		return -1;
	return nearbybacksta[i][0];
}
...
int Station::getNearbyBackpath(int i)
{
	if (i >= nearbybacknum)
		return -1;
	return nearbybacksta[i][1];
}
int Station::getNo()
{
	return staNo;
